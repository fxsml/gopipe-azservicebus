...
// TestOrdering_GoroutineDispatch shows that channel delivery order is non-deterministic in both
// peek and receive mode because each message is dispatched to its own goroutine. All goroutines
// race to write to msgChan; the winner depends on the scheduler, not the broker's message order.
func TestOrdering_GoroutineDispatch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := range numMessages {
				...
				rawMsgs[i] = message.NewRaw(
					[]byte(fmt.Sprintf(`{"id":%q}`, id)),
					message.Attributes{message.AttrID: id},
					nil,
				)
			}
			require.NoError(t, pub.PublishBatch(ctx, "test", rawMsgs...))

			sub, err := gosb.NewSubscriber(client, topicSub, "test", gosb.SubscriberConfig{
				MaxInFlight:      numMessages,
				EnablePeekMode:   tc.peekMode,
				PeekPollInterval: 250 * time.Millisecond,
			})
			require.NoError(t, err)
			subCtx, subCancel := context.WithCancel(ctx)
			...
			t.Logf("Published order: %v", publishedIDs)
			t.Logf("Received order:  %v", receivedIDs)
			assert.Equal(t, publishedIDs, receivedIDs, "received order should match publish order")
		})
	}
}
